...
//Unisex
showProgressDialog("Getting shoes...");
...
getMoreUnisexButton.onclick = () => {
    ...
    if (lastDownloadedUnisexShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more unisex shoes...");
            isDownloading = true;
            listAll(ref(storage, "products/shoes/unisex")).then(
                response => {
...
getMoreMaleButton.onclick = () => {
    ...
    if (lastDownloadedMaleShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more male shoes...");
            isDownloading = true;
            listAll(ref(storage, "products/shoes/male")).then(
                response => {
                    //if response is larger than last index + 21, get only 20
...
getMoreFemaleButton.onclick = () => {
    ...
    if (lastDownloadedFemaleShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more female shoes...");
            isDownloading = true;
            listAll(ref(storage, "products/shoes/female")).then(
                response => {
                    //if response is larger than last index + 21, get only 20
...
function addToCart(event) {
    ...
    */
    ...
    if (!orders || orders === "") {
        ...
        setCookie("orders", productName + "---" + productGender + "---" + "shoes", 90);
        setCookie("ordersCount", "1");
        ordersCount = 1;
    } else {
        //check if the poduct is there already
        if(orders.indexOf(productName + "---" + productGender + "---" + "shoes") >= 0) {
            alert("Sorry, product was already added!");
            return;
        }
        setCookie("orders", orders + ">>" + productName + "---" + productGender + "---" + "shoes", 90);
