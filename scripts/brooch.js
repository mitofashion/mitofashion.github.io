...
//Unisex
showProgressDialog("Getting ornaments...");
...
getMoreUnisexButton.onclick = () => {
    ...
    if (lastDownloadedUnisexShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more unisex ornaments...");
            isDownloading = true;
            listAll(ref(storage, "products/brooch/unisex")).then(
                response => {
...
getMoreMaleButton.onclick = () => {
    ...
    if (lastDownloadedMaleShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more male ornaments...");
            isDownloading = true;
            listAll(ref(storage, "products/brooch/male")).then(
                response => {
                    //if response is larger than last index + 21, get only 20
...
getMoreFemaleButton.onclick = () => {
    ...
    if (lastDownloadedFemaleShoe) {
        if (!isDownloading) {
            showProgressDialog("Getting more female ornaments...");
            isDownloading = true;
            listAll(ref(storage, "products/brooch/female")).then(
                response => {
                    //if response is larger than last index + 21, get only 20
...
function addToCart(event) {
    ...
    */
    ...
    if (!orders || orders === "") {
        ...
        setCookie("orders", productName + "---" + productGender + "---" + "brooch", 90);
        setCookie("ordersCount", "1");
        ordersCount = 1;
    } else {
        //check if the poduct is there already
        if(orders.indexOf(productName + "---" + productGender + "---" + "brooch") >= 0) {
            alert("Sorry, product was already added!");
            return;
        }
        setCookie("orders", orders + ">>" + productName + "---" + productGender + "---" + "brooch", 90);
